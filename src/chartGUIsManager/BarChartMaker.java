		...
		try{
			...
			for(int i=0;i<seriesOpt.size();i++){
				...
				if(xResults.size()!=yResults.size()){
					String warning="Invalid BarChart!xAxis column is not the same size as yAxis!";
					valid=false;
					showWarning(warning);
				}
				else{
					for(int j=0;j<xResults.size();j++){
						if(!xResults.get(j).equals(" ") && !yResults.get(j).equals(" ")){
							series.getData().add(new XYChart.Data<>(xResults.get(j), Double.parseDouble(yResults.get(j))));
						}
					}
